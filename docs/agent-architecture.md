...
Task 9 registers nine tools with an exact role matrix. Every execution re-fetches the current user and authoritative requirement detail through `BackendClient`. Draft tools require the correct handler/status and latest `version`; they never submit, reject, start purchase, submit to warehouse, or complete a requirement. The detailed contract is documented in `docs/agent-tools.md`.

...
状态：已实现。不依赖 LLM，支持开始采购、供应商搜索/详情/创建、采购字段部分保存、
后端实际总价、供应商档案同步明确确认、仓库管理员候选和提交仓库。
...
状态：已实现。需求人、楼长、采购员和仓库管理员的确定性端到端闭环已通过 Fake
Application E2E 封板，包含驳回重提和架构隔离回归。正式通知事件 Schema 与真实后端
smoke 仍是外部联调项。

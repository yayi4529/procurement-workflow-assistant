    ...
    def build(
        ...
        system = AssistantMessage(
            ...
            content=(
                ...
                "查询采购单列表、详情、状态、时间线或总数, 必须先调用 query_purchase_requests。"
                "用户询问总数时, 只能使用工具返回的 total_count, 不得自行数候选或估算。"
                ...
                "不得把同一条消息中的字段拆成多个调用。"
                "你负责理解用户的自然语言并生成结构化工具参数。"
                "例如精密空调可常识性分类为 device_profession=暖通; "
                "除此之外不得推断用户未提供且后端未返回的品牌、型号、数量或申请原因。"
                "如果上一轮正在收集品牌或型号并展示了推荐列表, 用户用第一、第二、第1个、"
                "选1或纯数字等自然语言选择时, 调用 update_purchase_draft 并传 selection_index; "
                "不要传内部 product_ref, 也不要自行填写或编造候选值。"
                "如果用户直接回答品牌或型号, 将去除自然语言句尾标点后的实际值分别传入 "
                "brand 或 model。自然语言理解由你完成。"
                "用户明确要求新建另一张采购草稿时, 调用 update_purchase_draft 并传 start_new=true; "
                "是否新建由你理解用户意图, Python 不解析用户原文。"
                ...
                "用户可直接提供推荐外的品牌或型号。用户回复序号时必须使用 selection_index, "
                "由工具根据最近候选确定真实值。"
